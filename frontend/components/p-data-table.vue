<template>
  <n-data-table
    ...
    ref="myTable"
    :flex-height="height != false"
    striped
    :scroll-x="disableScollX ? '100%' : 200 * columns.length"
    :row-key="(rowData) => id.map((i) => rowData[i]).join('-')"
    :row-props="rowProps"
...
var defaultRowProps = (rowData, rowIndex) => ({
  onClick: () => {
    if (props.selectable) {
...
var rowProps = computed(() =>
  props.rowProps ? props.rowProps : defaultRowProps
);
var myTable = ref(null);
function resetFilters() {
  myTable.value.clearFilters();
  props.columns.forEach((c) => (c.filterOptionValue = null));
}
function scrollTo(value) {
  myTable.value.scrollTo(value);
}
function getData() {
  return myTable.value.mainTableInstRef.bodyInstRef.rawPaginatedData.map(
    ({ __typename, ...o }) => o
  );
}
  ...
  props.columns.forEach((field) => {
      ...
      field.filter = (value, row) => {
        ...
        if (field.type == "Set")
          return (
            ...
            (v &&
              v.some(
                (va) => va.toLowerCase().indexOf(value.toLowerCase()) >= 0
              ))
          );
        return (
    ...
    if (field.type == "Currency") {
      ...
      field.render = (row) =>
        row[field.key] == 0
          ? "-"
          : row[field.key].toLocaleString("pt-br", {
